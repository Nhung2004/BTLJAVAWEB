...
public class DatabaseConfig
{
	...
	// Lấy đường dẫn của file Config
	static
	{
		// Kiểm tra đường dẫn của file DatabaseConfig.txt (Debug)
		System.out.println("Classpath resources:");
		URL url = DatabaseConfig.class.getClassLoader().getResource("");
		if(url != null)
		{
			System.out.println(url.getPath());
		}

		CONFIG_FILE_STREAM = DatabaseConfig.class.getClassLoader().getResourceAsStream("DatabaseConfig.txt");
		if(CONFIG_FILE_STREAM == null)
		{
			throw new RuntimeException("DatabaseConfig.txt not found in classpath resources!");
		}
	};
	...
	static
	{
		Map<String, String> config = readConfigFromFile(CONFIG_FILE_STREAM);
		...
		if(config != null)
		{
			...
			PASSWORD                 = config.getOrDefault("Password", "123456789");
			...
			CONNECTION_URL = String.format("jdbc:%s://%s:%s;databaseName=%s;encrypt=%s;trustServerCertificate=%s", WORKSPACE,
	...
	// Method đọc file Config
	private static Map<String, String> readConfigFromFile(InputStream fileStream)
	{
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(fileStream)))
		{
			...
			while ((line = reader.readLine()) != null)
			{
				...
				// Comments và cách dòng (Dòng trống)
