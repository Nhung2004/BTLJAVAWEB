...
<div class="product-item ${param.maTheLoai}">
	<div class="product product_filter">
		...
		<div class="product_image"
			style="width: 100%; height: 220px; overflow: hidden; display: flex; justify-content: center; align-items: center; background-color: #f9f9f9;">
			<img src="${param.image}" alt="${param.productName}"
	...
	<c:if test="${'true' == param.addToCart}">
		<form action="${pageContext.request.contextPath}/addGioHang"
			method="post">
			<input type="hidden" name="productId" value="${param.productId}">
			<input type="hidden" name="productName" value="${param.productName}">
			<input type="hidden" name="price" value="${param.price}"> <input
				type="hidden" name="quantity" value="1"> <input
				type="hidden" name="image" value="${param.image}">
